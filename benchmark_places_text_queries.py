#!/usr/bin/env python3
"""
Google Places API Benchmark Script - Text Queries
==================================================
Standalone script to test predefined places_text_queries against Google Places API v1.
No project imports - uses only standard library + httpx + python-dotenv.

Output: Single TXT log file with all results per query.
"""
...
SEARCH_PLAN = {
    ...
    "places_text_queries": [
        ...
        "AI powered SEO agencies",
        "AI SEO software companies",
        "SEO technology startups",
...
def create_log_file() -> Path:
    ...
    log_file = logs_dir / f"benchmark_text_queries_{timestamp}.txt"
...
def write_log_header(f, search_plan: Dict[str, Any], location_str: str):
    ...
    """
    f.write("=" * 80 + "\n")
    f.write("GOOGLE PLACES API BENCHMARK - TEXT QUERIES\n")
    f.write("=" * 80 + "\n")
    ...
    f.write(f"Number of Text Queries: {len(search_plan['places_text_queries'])}\n")
    f.write("=" * 80 + "\n\n")
...
def write_query_results(
    ...
    """
    Write results for a single text query.
    """
    f.write("=" * 80 + "\n")
    f.write(f"QUERY #{query_num}: {text_query}\n")
    f.write("=" * 80 + "\n")
...
def main():
    ...
    """
    print("=" * 80)
    print("GOOGLE PLACES API BENCHMARK - TEXT QUERIES")
    print("=" * 80)
    print()
    ...
    if not location_restriction:
        ...
        with open(log_file, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write("GOOGLE PLACES API BENCHMARK - TEXT QUERIES - ERROR\n")
            f.write("=" * 80 + "\n")
            f.write(f"Timestamp: {datetime.now(timezone.utc).isoformat()}\n")
            f.write(f"Request ID: {SEARCH_PLAN['request_id']}\n")
            f.write("\n")
    ...
    # Get text queries from search plan
    text_queries = SEARCH_PLAN.get("places_text_queries", [])
    
    if not text_queries:
        print("ERROR: No places_text_queries found in search plan")
        sys.exit(1)
    
    print("=" * 80)
    print("TEXT QUERIES TO TEST")
    print("=" * 80)
    for idx, query in enumerate(text_queries, 1):
        print(f"{idx}. {query}")
    print()
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        ...
        # Execute each text query
        print("=" * 80)
        print("EXECUTING TEXT QUERIES")
        print("=" * 80)
        
        for query_num, text_query in enumerate(text_queries, 1):
            print(f"\n[{query_num}/{len(text_queries)}] {text_query}")
