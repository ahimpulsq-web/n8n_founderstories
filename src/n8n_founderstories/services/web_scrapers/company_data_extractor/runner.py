...
def _ensure_output_columns_at_end(gsm: GoogleSheetsManager, tab: str, header: list[str]) -> list[str]:
    ...
    # Re-apply formatting (auto-resize new columns)
    sid = gsm.client.get_sheet_id(tab)
    if sid is not None:
        reqs = gsm.client.build_header_formatting_requests(sheet_id=sid, n_cols=len(new_header))
        for r in reqs:
            gsm.queue_batch_update_request(r)
...
async def _extract_batch_with_fetcher(
    *,
    ...
    domains: list[str],
) -> list[tuple[list[str], Optional[str], Optional[str], list[tuple[str, str]], Optional[str], Optional[str], str, str]]:
    async def one(d: str) -> tuple[list[str], Optional[str], Optional[str], list[tuple[str, str]], Optional[str], Optional[str], str, str]:
...
def run_email_extractor_job(
    *,
    ...
    try:
        ...
        # EmailExtractor_State is a special state-tracking tab (not a tool output tab)
        ...
        while processed < max_updates:
            ...
            results: list[tuple[list[str], Optional[str], Optional[str], list[tuple[str, str]], Optional[str], Optional[str], str, str]] = []
            if valid_domains:
            ...
            mapped: dict[int, tuple[list[str], Optional[str], Optional[str], list[tuple[str, str]], Optional[str], Optional[str], str, str]] = {}
            ...
            for i in range(eff_n):
                ...
                if not domain:
                    ...
                    emails_unique, best_email, best_src, pairs, contact_name_raw, company_desc_raw, reason0, debug0 = mapped.get(
                        i, ([], None, None, [], None, None, "fetch_failed", "no_result")
                    )

                    best = best_email or ""
                    all_emails = _format_email_pairs(pairs) if pairs else ""
                    src = best_src or ""
                    contact_name = contact_name_raw or ""
                    company_description = company_desc_raw or ""
        ...
        if apply_formatting and sheets:
            try:
                ...
                sheets.format_table_layout(
                    ...
                    header_row=True,
                    auto_resize=True,
                    # Use CLIP to prevent excessive row heights, but allow newlines to show
                    wrap_strategy_body="CLIP",
                    header_height=settings.google_sheets_header_row_height,
                    body_row_height=settings.google_sheets_body_row_height,
                )
            except Exception:
