...
def extract_impressum_to_end(md: str) -> str:
    """
    Extract markdown content starting from an 'Impressum/Imprint' header to end-of-document.
    Returns empty string if no Impressum header is found.
    """
    if not md:
        return ""
    lines = md.splitlines()
    ...
    for line in lines:
        if not in_section:
            if IMPRINT_START_RE.match(line):
                in_section = True
                out.append(line)
            continue
        out.append(line)
