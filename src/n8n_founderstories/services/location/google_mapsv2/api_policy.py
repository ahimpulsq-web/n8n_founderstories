...
class GoogleMapsAPIPolicy:
    ...
    def call(self, *, fn: Callable[[], Any], request_id: str | None = None, label: str = "API") -> Any:
        ...
        (e.g. lambda: client.geocode(...) or lambda: client.search_text(...)).
            ...
            try:
                ...
                if status == 403 or status == 429 or 500 <= status <= 599:
                    ...
                    if retry_after is not None:
                        logger.warning(
                            "GMAPSV2 | RETRY_AFTER | label=%s | request_id=%s | status=%s | retry_after_s=%.2f | attempt=%d/%d",
                        ...
                        if status == 403:
                            wait_time = 30.0
                            logger.warning(
                                "GMAPSV2 | QUOTA_WAIT | label=%s | request_id=%s | status=%s | wait_s=%.2f | attempt=%d/%d",
                                label, request_id, status, wait_time, attempt, self.retry.max_attempts
                            )
                            time.sleep(wait_time)
                        else:
                            backoff_time = self.retry.base_delay_s * (2 ** (attempt - 1))
                            logger.warning(
                                "GMAPSV2 | RETRY | label=%s | request_id=%s | status=%s | backoff_s=%.2f | attempt=%d/%d",
                ...
                last_exc = e
                logger.warning(
                    "GMAPSV2 | RETRY_NET | label=%s | request_id=%s | err=%s | attempt=%d/%d",
