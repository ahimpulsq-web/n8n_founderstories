"""
Hunter.io source adapter for MasterV2.

Fetches normalized candidates from htr_results table.
...
def fetch_candidates(job_id: str, conn: psycopg.Connection[Any], sheet_id: str | None = None) -> list[LeadCandidate]:
    """
    Fetch lead candidates from htr_results for a given job_id.
    
    Returns normalized candidates without sorting.
    Sorting is applied at the sheets export level to match HunterIO sheet ordering.
    ...
    with conn.cursor() as cur:
        ...
        for domain, organization, request_id in cur.fetchall():
            ...
            try:
                ...
                logger.warning(
                    "MASTER | source=hunter | job_id=%s | skip_invalid_domain=%s | error=%s",
                    job_id,
                    domain,
                    str(e)
                )
    ...
    logger.info(
        "MASTER | source=hunter | job_id=%s | fetched=%d candidates",
        job_id,
        len(candidates)
    )
